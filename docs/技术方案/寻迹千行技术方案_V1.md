...
CREATE TABLE order_main (
    ...
    -- 订单状态
    status TINYINT NOT NULL DEFAULT 0 COMMENT '订单状态: 0待支付 1已预订 2出行中 3已完成 4已取消 5退款申请中 6已退款 7已关闭',
...
| 添加备注 | POST | /admin/order/remark | 添加管理员备注 |
| 订单统计 | GET | /admin/order/stats | 各状态订单数及销售额 |
...
| 状态码 | 枚举名 | 名称 | 说明 |
|--------|--------|------|------|
| 0 | PENDING_PAY | 待支付 | 订单创建，等待支付 |
| 1 | BOOKED | 已预订 | 支付成功，预订确认 |
| 2 | TRAVELING | 出行中 | 行程进行中 |
| 3 | COMPLETED | 已完成 | 行程结束 |
| 4 | CANCELLED | 已取消 | 用户取消或超时 |
| 5 | REFUND_APPLY | 退款申请中 | 申请退款，待审核 |
| 6 | REFUNDED | 已退款 | 退款完成 |
| 7 | CLOSED | 已关闭 | 系统关闭 |
